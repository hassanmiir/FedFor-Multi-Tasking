...
from tensorflow.keras.layers import LSTM, GRU, Dense, Attention, TimeDistributed, Conv1D, MaxPooling1D, Dropout, Flatten, RepeatVector, Input
...
# Instead of using TimeDistributed, let's integrate it into the Conv1D layers
...
x = GRU(100, activation='relu', dropout=0.25, return_sequences=True, kernel_regularizer=regularizers.l2(0.001))(x)

output = TimeDistributed(Dense(3, activation='sigmoid'))(x)  

model = Model(inputs=input_layer, outputs=output)

optimizer = tfa.optimizers.AdamW(learning_rate=0.0001, weight_decay=0.000002)
model.compile(optimizer=optimizer, loss='mse')
model.summary()

# Set the MLflow experiment
mlflow.set_experiment("FedFor_GRU_D1")
